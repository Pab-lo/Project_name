...
#     data_test_df = data_test_df[:10]
    ...
    for pred_idx in range(arima_start, X_train.shape[0]):
        ...
        model = ARIMA(X_running, order = (p, 1, q))
...
for p in [0, 1, 2]:
    for q in [0, 1, 2]:
        ...
        print(f"{p}, {q} done: {100.*np.mean(all_alphas['Val'])}")
...
print_model_results_table(all_pairs, all_alphas, all_sharpes, f"ARIMA results", f"tab:arima_results")
